...
class Game(db.Model):
    ...
    def apply_movement(self, player_new_x, player_new_y, new_boxes):
        ...
        La fonction vérifie quel joueur est le joueur actuel (current_player) et met à jour les coordonnées du joueur correspondant .
        Ensuite, elle passe le tour à l’autre joueur. La grille (boxes) est également mise à jour, et les changements 
        sont sauvegardés en base de données.
...
class Q_table_test(db.Model) :
    ___tablename__ = "q_table_test"

    ''' 
        je ne sais pas comment faire, il faut faire en sorte de lier la classe game et cette classe
        - nécessaire ? 
        il suffit de c'est attribut et c'est bon, peut être mettre ce changement dans 
        la fonction apply_movement
    '''

    current_player_x = db.Column(db.Integer, db.ForeignKey('game.current_player'), primaryKey=True, nullable=False)
    current_player_y = db.Column(db.Integer, nullable=False)

    other_player_x = db.Column(db.Integer, nullable=False)
    other_player_y = db.Column(db.Integer, nullable=False)



    expectation_right = db.Column(db.Integer, nullable=True)
    expectation_left = db.Column(db.Integer, nullable=True)
    expectation_up = db.Column(db.Integer, nullable=True)
    expectation_down = db.Column(db.Integer, nullable=True)

    def __init__ (self,id_game, state_board, current_player_x, current_player_y, other_player_x, other_player_y, expectation_right, expectation_left, expectation_up, expectation_down):
        self.id_game = id_game
        self.state_board = state_board
        self.current_player_x = current_player_x
        self.current_player_y = current_player_y
        self.other_player_x = other_player_x
        self.other_player_y = other_player_y
        self.expectation_right = expectation_right
        self.expectation_left = expectation_left
        self.expectation_up = expectation_up
        self.expectation_down = expectation_down 
